...
Безпосередньо перед записом HTML у файл `generate()` виводить три аудит-
таблиці у лог (orphans, down-state, lt-links) — по одному рядку на запис,
щоб їх можна було швидко знайти `grep`-ом, не відкриваючи сам звіт. Формат
лога повторює колонки таблиці; lt-рядки сортуються за `(router,
sideA.iface)`. Текстові маркери `(-)` потрапляють у лог як є; `(!)` і
кольорові `<span>`-обгортки — лише в HTML.
